...
const supportedIdPrefixes = (value) => ({
    "https://d-nb.info/gnd/": { url: `https://lobid.org/gnd/search?format=json&q=id:"${value}"`, test: (r) => r.member && r.member.length > 0, process: (r) => r.member[0].preferredName },
    "http://rppd.lobid.org/": { url: `https://rppd.lobid.org/search?format=json&q=rppdId:"${lastSegment(value)}"+OR+gndIdentifier:"${lastSegment(value)}"`, test: (r) => r.member && r.member.length > 0, process: (r) => r.member[0].preferredName },
    "http://lobid.org/resources": { url: `https://lobid.org/resources/${lastSegment(value)}.json`, test: (r) => r, process: (r) => r.title },
    "http://rpb.lobid.org": { url: `https://rpb.lobid.org/${lastSegment(value)}?format=json`, test: (r) => r.member && r.member.length > 0, process: (r) => r.member[0].title },
...
module.exports = {
    ...
    labelFor: async (component) => {
        if (component.subjectComponent) {
            ...
            const value = component.value.replace(/(http.+):\s.+$/g, '$1');
